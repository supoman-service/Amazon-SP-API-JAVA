...
public class ReportsApi {
     ...
     * @date 2021/5/26
     * @param body
     * @return com.amazon.spapi.model.reports.CreateReportResponse
     */
   /* public CreateReportResponse createReport(CreateReportSpecification body) throws ApiException {
     ...
     */
    public String getReportByDocumentId(String reportDocumentId) throws ApiException {
        ApiResponse<GetReportDocumentResponse> resp = getReportDocumentWithHttpInfo(reportDocumentId);
        var response= resp.getData();
        var key= response.getPayload().getEncryptionDetails().getKey();
        var iv=response.getPayload().getEncryptionDetails().getInitializationVector();
        var download=new DownloadDocument();
        var compressionAlgorithm=response.getPayload().getCompressionAlgorithm();
        String zip=null;
        if (compressionAlgorithm!=null){
            zip=compressionAlgorithm.getValue();
        }

        return download.downloadAndDecrypt(key,iv,response.getPayload().getUrl(),zip);
    ...
    /**
     * 第二步：获取请求报告生成的列表信息
     *返回与您指定的过滤器相匹配的报告的报告详细信息。 **使用计划：** |费率（每秒请求数）|爆裂| ---- | ---- | | 0.0222 | 10 |有关更多信息，请参见“使用计划和费率限制”。在销售伙伴API文档中。
     * @param reportTypes 用于过滤报告的报告类型列表。当提供reportTypes时，还可以提供其他过滤器参数（processingStatuses，marketplaceIds，createdSince，createdUntil）和pageSize。 reportTypes或nextToken是必需的。 （可选的）
     * @param processingStatuses 用于过滤报告的处理状态列表。 （可选的）
     * @param marketplaceIds 用于过滤报告的市场标识符列表。返回的报告将至少匹配您指定的市场之一。 （可选的）
     * @param pageSize 单个调用中要返回的最大报告数。 （可选，默认为10）
     * @param createdSince 最早的报告创建日期和时间，报告以ISO 8601日期时间格式包含在响应中。默认值为90天前。报告最多保留90天。 （可选的）
     * @param createdUntil 最新的报告创建日期和时间，报告将以ISO 8601日期时间格式包含在响应中。默认为现在。 （可选的）
     * @param nextToken 在对上一个请求的响应中返回的字符串令牌。当结果数超过指定的pageSize值时，将返回nextToken。要获取下一页结果，请调用getReports操作并将此标记作为唯一参数。与其他任何参数一起指定nextToken将导致请求失败。 （可选的）
     ...
     */
    public static ReportsApi amazonAuthorizationGrantReportsApi(AmazonAuthorConfig authorConfigDTO) {
